...
public class AttackingSpell extends Spell {
	...
	public AttackingSpell(String name, int manaCost, MagicLevel levelNeeded, boolean type, boolean percentage, int amount) {
		...
		if (amount < 0)
			throw new IllegalArgumentException("amount has to be non negative");
	...
	*/
	...
	public void doEffect(MagicEffectRealization target) {
		Set<AttackingSpell> setOfAttacks = new HashSet<>();
		setOfAttacks.add(this);
		
		if (target.isProtected(this))
			target.removeProtection(setOfAttacks);
		else {
			if (type) {
				...
				if (percentage) target.weakenMagicPercent(amount);
